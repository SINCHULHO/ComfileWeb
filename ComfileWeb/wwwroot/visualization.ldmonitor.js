function createVisualizationLdMonitor(context) {
	...
	function requestJson() {
		...
		if (typeof context.getLdMonitorDocument === 'function') {
			const document = context.getLdMonitorDocument();
			if (document) {
				onLdJsonResult(document);
				return;
			}

			state.ldJson = {
				success: false,
				detail: context.getUseKoreanLanguage() ? 'LD 데이터가 없습니다. 주소 테이블에서 CUBLOC2 프로젝트 파일을 가져오세요.' : 'No LD data. Import a CUBLOC2 project file from the address table.'
			};
			renderDiagram();
			return;
		}
